...
- Improved Boilermaker API integration which allows the system to run the Boilermaker more like a digital SCR controller. As in, the boiler temperature is allowed to naturally rise over time, but is more precisely controlled within the preset window.
...
- Heating stepper motor control updated to use 5% of total motor steps rather than using the 10% heat-jump positions for boiler temperature management during a distillation run. This is so stepper motor heating control operation is more like Boilermaker operation.
